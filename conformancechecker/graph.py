...
from activity import DCRActivityBase, DCRActivityNest, DCRActivity
from conn import DCRConnection, ConnectionTypes, Condition, Milestone
...
class DCRGraph(object):
    ...
    def __init__(self, xml_path):
        ...
        self.Connections = []
        self.Mappings = {}
        self.Nodes = [] #Activities, nodes, events seem to be the same
        self.Expressions = []
        ...
        self.create_expressions_dcr_graph()
        ...
        for constraint in self.dcr_xml_root.iter('constraints'):
            for constraint_type in constraint:
                for connection in constraint_type:
                    ...
                    if 'expressionId' in connection.attrib:
                        expression_id = connection.get('expressionId')
                        expression = self.get_expression(expression_id)
    ...
    def get_expression(self, expression_id):
        """
        Gets the expression by the id
        :param expression_id: id that is looked for
        :return: The respective expression
        """
